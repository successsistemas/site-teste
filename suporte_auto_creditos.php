...
<br>
Exclui créditos anteriores a: <strong><? echo date('m/Y', strtotime($geral_credito_acumulo_data)); ?></strong>
</div>

<? if($totalRows_manutencao_dados > 0){ ?>

	<?
    // update (atualiza créditos com mais de '6' meses para 'expirado')
    $updateSQL = sprintf("
                         UPDATE geral_credito SET status = '0' 
                         WHERE data_criacao < %s and status = 1 and data_utilizacao IS NULL",
                         GetSQLValueString($geral_credito_acumulo_data.'-01 00:00:00', "date"));					
    mysql_select_db($database_conexao, $conexao);
    $Result = mysql_query($updateSQL, $conexao) or die(mysql_error());
    // fim - update (atualiza créditos com mais de '6' meses para 'expirado')

    // update (atualiza créditos com adiantamento do mês anterior)
    $updateSQL_adiantamento = sprintf("
                         UPDATE geral_credito SET adiantamento = 'n' 
                         WHERE data_criacao = %s and adiantamento = 's'",
    ...
    // fim - update (atualiza créditos com adiantamento do mês anterior)

	// delete (deleta geral_contrato_alterado a mais de 2 meses)
	$deleteSQL_geral_contrato_alterado = "
                         DELETE FROM geral_contrato_alterado 
                         where data_alteracao < (ADDDATE(LAST_DAY(SUBDATE(CURDATE(), INTERVAL 2 MONTH)), 1))";				
    mysql_select_db($database_conexao, $conexao);
    $Result_geral_contrato_alterado = mysql_query($deleteSQL_geral_contrato_alterado, $conexao) or die(mysql_error());
	// fim - delete (deleta geral_contrato_alterado a mais de 2 meses)
    ?>
    ...
    <div style="border: 1px solid #CCC; margin: 5px; padding: 5px;">
        ...
        <strong>
        <?php 
        if($row_manutencao_dados['status1']=="0"){echo "Desbloqueado";} // manual
        if($row_manutencao_dados['status1']=="1"){echo "<font color='red'>Bloqueado</font>";} // manual
        ?> | 
        </strong>
        Status automático: 
        <strong>
        <?php 
        if($row_manutencao_dados['flag1']=="0"){echo "Desbloqueado";} // autom
        if($row_manutencao_dados['flag1']=="1"){echo "<font color='red'>Bloqueado</font>";} // autom
        ?>
        </strong>
        <br>
        <!-- fim - dados -->
        ...
        <? if($row_manutencao_dados['datatv17'] > 0 and $row_manutencao_dados['datatv17'] >= date("Y-m-01", strtotime("-1 month" )) and ($contrato_tipo_visita=="3" or $contrato_tipo_visita=="4")){ ?>
            ...
            SELECT count(IdContratoAlterado) as retorno  
            FROM geral_contrato_alterado 
            WHERE contrato = %s and data_alteracao = %s 
            ORDER BY IdContratoAlterado DESC", 
            ...
            <? if($row_geral_contrato_alterado['retorno'] == 0){ ?>
                ...
                INSERT INTO geral_contrato_alterado (contrato, data_alteracao, tipo_visita, data_criacao) 
                VALUES (%s, %s, %s, %s)", 
                GetSQLValueString($row_manutencao_dados['codigo17'], "text"), 
                GetSQLValueString($row_manutencao_dados['datatv17'], "date"),
		...
		<?
        // 1 ou 5 ---------------------------------------------------------------------------------------------------------------
        if($contrato_tipo_visita=="1" or $contrato_tipo_visita=="5"){
		?>
        ...
        <?
        // 3M-4T -------------------------------------------------------------------------------------------------------------
        if($contrato_tipo_visita=="3" or $contrato_tipo_visita=="4"){
		?>
            ...
            <? } else { ?>
                ...
                if($totalRows_geral_credito_ultimo > 0 and $row_geral_credito_ultimo['data_criacao']!=""){
					...
					if($row_manutencao_dados['optacuv17']=="N"){
						...
						DELETE FROM geral_credito
						WHERE contrato = %s and status = 1 and IdCredito <> %s and data_criacao < %s", 
						GetSQLValueString($row_manutencao_dados['codigo17'], "text"),
            ...
            <? if($data_criacao=='0000-00-00 00:00:00'){ echo "Não existe"; }else{ echo date('m/Y', strtotime($data_criacao)); } ?>
            </strong></span>
            <br>
            ...
            ?>
                ...
                $insertSQL = sprintf("INSERT INTO geral_credito (contrato, tipo_visita, data_criacao, status) VALUES (%s, %s, %s, %s)",
                   GetSQLValueString($row_manutencao_dados['codigo17'], "text"),
                   GetSQLValueString($row_manutencao_dados['visita17'], "text"),
                   GetSQLValueString(date('Y-m-d H:i:s'), "date"),			
				...
				if($row_consulta_contrato_alterado['status'] == 1){
					...
					UPDATE geral_contrato_alterado SET status = '0' 
					WHERE contrato = %s", 
					GetSQLValueString($row_manutencao_dados['codigo17'], "text"));
					mysql_select_db($database_conexao, $conexao);
