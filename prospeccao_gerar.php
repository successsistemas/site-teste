...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "prospeccao")) {
	...
	if($cliente_novo_antigo=="a"){
		...
		$cpf_cnpj = shellDescriptografa($row_cliente_antigo_selecionado['cgc1']);
		$rg_inscricao = shellDescriptografa($row_cliente_antigo_selecionado['insc1']);
...
$(document).ready(function() {
	...
	$("input[name=cpf_cnpj]").blur(function() {
		...
		if (cpf_cnpj.length == 11) { // utilizar validação do CPF
			...
			if(retorno==false){
				$('#cpf_cnpj_erro table tr td').html("CPF Inválido!");
				$('#cpf_cnpj_erro').show();
			...
			if(retorno==false){
				$('#cpf_cnpj_erro table tr td').html("CNPJ Inválido!");
				$('#cpf_cnpj_erro').show();
				setTimeout(function(){ cpf_cnpj_campo.focus()}, 50);
				return false;
			} else {
			...
			$('#cpf_cnpj_erro table tr td').html('Tamanho inválido');
			$('#cpf_cnpj_erro').show();
			setTimeout(function() {cpf_cnpj_campo.focus();}, 50);
			return false;
		}
		...
		if(consulta_cpf_cnpj == 1){
			...
			function(retorno){
				...
				if(retorno.contador > 0){
					var texto_retorno = "CPF/CNPJ já cadastrado. Contrato Nº: "+retorno.contrato+". Caso deseje prosseguir, continue inserindo as informações ou <a style='color: red; font-weight:bold;' href='prospeccao.php?padrao=sim&<? echo $prospeccao_padrao; ?>'>clique aqui para sair</a>.";
					$('#cpf_cnpj_erro table tr td').html(texto_retorno);
					$('#cpf_cnpj_erro').show();
					//setTimeout(function() {cpf_cnpj_campo.focus();}, 50);
					//return false;
				}
...
<div class="cpf_cnpj_erro div_solicitacao_linhas4" id="cpf_cnpj_erro">
	<table cellspacing="0" cellpadding="0" width="945">
		<tr>        
