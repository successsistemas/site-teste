...
// tamanho janela agenda
$agenda_editar_tabela_width = 770;
$agenda_editar_tabela_height = 500;

// situacao padrao de solicitacao
$situacao_padrao = "situacao[]=criada&situacao[]=recebida&situacao[]=em análise&situacao[]=analisada&situacao[]=em orçamento&situacao[]=aprovada&situacao[]=em execução&situacao[]=executada&situacao[]=em testes&situacao[]=testada&situacao[]=em validação&tipo[]=Correção&tipo[]=Fiscal&tipo[]=Implementação";
...
function shellDescriptografa($valor){
	...
	if ($_SERVER['SERVER_NAME'] <> "localhost") {
		return shell_exec("./shell/hb_decrypt " . escapeshellarg($valor) . " " . $chave . " 2>&1");
	} else {
		return $valor;
	}
...
function shellCriptografa($valor){
	...
	if ($_SERVER['SERVER_NAME'] <> "localhost") {
		return shell_exec("./shell/hb_crypt " . escapeshellarg($valor) . " " . $chave . " 2>&1");
	} else {
		return $valor;
	}
