...
SELECT 
    ...
    case 
		when 
			(solicitacao.status = 'pendente solicitante' and solicitacao.previsao_validacao IS NOT NULL and solicitacao.previsao_validacao <> '0000-00-00 00:00:00') 
		then 
			ADDDATE(solicitacao.previsao_validacao, INTERVAL solicitacao_tipo_solicitacao.prazo_validacao_solicitacao_dias DAY)
    else solicitacao.previsao_validacao end 
	) AS previsao_validacao 
				...
				<td style="padding-top: 5px; padding-bottom: 5px;">
					...
					<div style="text-align:right; float: right">
						...
						<? if ($row_solicitacao['situacao'] == "em validação" and $row_solicitacao['previsao_validacao'] != "0000-00-00 00:00:00") { ?>
							...
							<? echo formataDataPTG($row_solicitacao['previsao_validacao_inicio']); ?> 
							à 
							<? echo formataDataPTG($row_solicitacao['previsao_validacao']); ?>
							<? if (strtotime($row_solicitacao['previsao_validacao']) < strtotime(date("Y-m-d H:i:s"))) { ?>
								| <span class="label_solicitacao">Atraso: </span>
								<?
								// dias_atraso ...
								$data_ini = strtotime($row_solicitacao['previsao_validacao']);
								$data_final = strtotime(date("Y-m-d H:i:s"));
				...
				<tr>
					...
					<td align="left">
						...
						if (
							...
							or
							// testador =======================================================================================================================================
							(($row_usuario['solicitacao_testador'] == 'Y' and $row_solicitacao['id_testador'] == $row_usuario['IdUsuario']) and (
								$row_solicitacao['situacao'] == "em testes"
							))
							// fim - testador =================================================================================================================================
						) { ?>
					...
					<td align="right" style="color:#F00; font-weight:bold;">
						...
						if (
							(
								...
								// testada -----------------------------------------------------------------------------	
								$row_solicitacao['situacao'] == "testada" and (
									(
										($row_solicitacao['id_usuario_responsavel'] == $row_usuario['IdUsuario']) // solicitante 
										and ($row_solicitacao['status_recusa'] != "1" and
											$row_solicitacao['previsao_validacao'] == "0000-00-00 00:00:00" and $row_solicitacao['id_usuario_responsavel'] != "" // solicitado validação
										)) or (

										($row_usuario['solicitacao_testador'] == "Y" and $row_solicitacao['id_testador'] == $row_usuario['IdUsuario']) and  // testador logado
										$row_solicitacao['status_recusa'] == "1"))
								// fim - testada -----------------------------------------------------------------------------	
								...
								($row_solicitacao['status'] == "devolvida para testador" and $row_solicitacao['id_testador'] == $row_usuario['IdUsuario']) or // devolução para testador
								($row_solicitacao['status'] == "devolvida para solicitante" and $row_solicitacao['id_usuario_responsavel'] == $row_usuario['IdUsuario']) // devolução para solicitante (validação)
							)
						) {
						?>
						...
						<? if ($row_solicitacao['situacao'] == "testada") { ?>
							<? if (
								$row_solicitacao['previsao_validacao'] == "0000-00-00 00:00:00" and
								$row_solicitacao['id_usuario_responsavel'] != "" and
								$row_solicitacao['status_recusa'] != "1"
							) { ?>

								Aguardando aceitação do solicitante
