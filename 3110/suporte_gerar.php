...
SELECT 
codigo1, nome1, fantasia1, contato1, endereco1, bairro1, cidade1, uf1, telefone1, comercio1, cep1, ultcompra1, atraso1, status1, flag1, tipo1, atraso1 
FROM da01 
...
SELECT 
	suporte_formulario.data 
FROM 
	suporte_formulario 
WHERE 
	suporte_formulario.contrato = %s and 
	suporte_formulario.status_flag = 'a' and 
	suporte_formulario.visita_bonus='s'
ORDER BY 
	suporte_formulario.IdFormulario DESC 
LIMIT 1 
", 
GetSQLValueString($colname_contrato, "text"));
	...
	if (((isset($_POST["MM_update"])) and ($_POST["MM_update"] == "suporte")) or ((isset($_GET["MM_update"])) and ($_GET["MM_update"] == "suporte"))) {
			...
			if($tipo_suporte_inloco == "cs"){
				...
				if($tipo_formulario == "Treinamento"){
					...
					if(
						...
						(
							(
								...
								$row_manutencao_dados['visita17'] == "4" and // trimestral
					...
					} else if($creditos == 0){ // não

						$status_flag = "b"; // bloqueado
						$status_flag_formulario = "b"; // bloqueado
						$situacao_formulario = "bloqueado";

					} else if($creditos >= 1 and $suporte_formulario_abater_credito == "n"){
				...
				$dados_suporte = array(
						...
						"prioridade_justificativa" => $prioridade_justificativa,
...
$(document).ready(function() {
	...
	<? } else if($tipo_suporte_inloco == "r"){ ?>
		...
		$('textarea[id="orientacao"]').attr('disabled', true);
		$('select[id="suporte_tipo_recomendacao"]').attr('disabled', true);
		$('select[id="prioridade"]').attr('disabled', true);
	...
	<? if($tipo_suporte_inloco == "cs"){ ?>
		...
		<? } ?>
		$("label[id=label_prioridade] > #req").show();
		$("label[id=label_prioridade] > #req").show();
	...
	$("select[id='prioridade']").change(function () { // ao mudar o valor do select 'prioridade'
		$("select[id='prioridade'] option:selected").each(function () {
			...
			if( prioridade_atual=="Alta" ){
				...
				var regra_prioridade_justificativa = "length>=10,prioridade_justificativa,Informe a justificativa da prioridade Alta com no mínimo 10 caracteres.";
				...
				rules.push("length>=10,prioridade_justificativa,Informe a justificativa da prioridade Alta com no mínimo 10 caracteres."); // adiciona a regra
...
<table cellspacing="0" cellpadding="0" width="100%" style="margin-top: 10px; margin-bottom: 10px;" id="prioridade_justificativa_caixa">
  <tr>
