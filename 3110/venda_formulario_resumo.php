...
$query_venda_agenda_treinamento =  sprintf("
									   ...
									   WHERE id_venda = %s and (status = 'a' or status = 'f')
...
<table width="100%" height="20" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="titulo_formulario" style="font-weight: bold">
      RESUMO DA IMPLANTAÇÃO E TREINAMENTO (<? echo $_GET['id_venda']; ?>)
    </td>
  </tr>
</table>
...
<tr>
        ...
        $tDias   = ($treinamento_tempo_segundo) / (3600*24);  // dias
        $tHoras = (($treinamento_tempo_segundo) % (3600*24)) / 3600; // horas
        $tMinutos = (	(($treinamento_tempo_segundo) % (3600*24)) % 3600	) / 60; // minutos
        
        echo sprintf('%02dd %02dh %02dm', $tDias, $tHoras , $tMinutos);
        ?>
        </td>
        ...
        <br>
        <? // if($row_venda['data_contrato']!=""){echo date('d-m-Y', strtotime($row_venda['data_contrato']));} ?>
        </td>
        
        <td width="20%">
        Horas de implantação adquiridas:
        <br>
		<? 
		$implantacao_tempo_segundo = $row_venda['implantacao_tempo']*60;
        $tDias   = ($implantacao_tempo_segundo) / (3600*24);  // dias
        $tHoras = (($implantacao_tempo_segundo) % (3600*24)) / 3600; // horas
        $tMinutos = (	(($implantacao_tempo_segundo) % (3600*24)) % 3600	) / 60; // minutos
...
<? 
$treinamento_tempo_hora_arredonda = ceil($treinamento_tempo_segundo = $row_venda['treinamento_tempo']/60);
for ($i = 1; $i <= ($treinamento_tempo_hora_arredonda*2); $i++) { 
?>
<tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td style="text-align:center">
    _____:_____ até_____:_____=_________
    </td>
    <td>&nbsp;</td>
  </tr>
<? } ?>
