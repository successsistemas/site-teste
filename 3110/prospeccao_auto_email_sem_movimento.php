...
FROM prospeccao
WHERE prospeccao.status_flag = 'a'
...
$prazo_prospeccao_auto_email_dias = $row_parametros['prospeccao_auto_email_sem_movimento_dias'];
...
<? if($totalRows_prospeccao > 0 and $prazo_prospeccao_auto_email_dias > 0){ ?>
	...
	<?php do { ?>
        ...
        mysql_select_db($database_conexao, $conexao);
        $query_prospeccao_descricoes = sprintf("
                                               SELECT id, data 
                                               FROM prospeccao_descricoes 
                                               WHERE id_prospeccao = %s and id_usuario_responsavel <> '' 
                                               ORDER BY id DESC LIMIT 1", 
                                               GetSQLValueString($row_prospeccao['id'], "int"));
        $prospeccao_descricoes = mysql_query($query_prospeccao_descricoes, $conexao) or die(mysql_error());
        $row_prospeccao_descricoes = mysql_fetch_assoc($prospeccao_descricoes);
        $totalRows_prospeccao_descricoes = mysql_num_rows($prospeccao_descricoes);
        // fim - prospeccao_descricoes
        ?>
        
        <? if($totalRows_prospeccao_descricoes > 0){ ?>
        <div style="border: 1px solid #CCC; margin: 5px; padding: 5px;">
        
            <!-- dados -->
            <?
            $previsao_geral = $row_prospeccao_descricoes['data'];
            ...
            <br>
            Número da descrição: <strong><?php echo $row_prospeccao_descricoes['id']; ?></strong>
            <br>
            Data da prospecção: <? echo date('d-m-Y  H:i', strtotime($row_prospeccao['data_prospeccao'])); ?>
            <br>
            ...
            <? if ($previsao_geral_passados_dias >= $prazo_prospeccao_auto_email_dias){ ?>
                ...
                <br>
                Esta prospecção completou mais de <? echo $prazo_prospeccao_auto_email_dias; ?> dia(s) sem movimento. Por favor verifique.
                </div>
                <?
                // atualiza prospecção
                ...
                $dados_prospeccao_descricao = array(
                        ...
                        "descricao" => "Esta prospecção completou mais de ".$prazo_prospeccao_auto_email_dias." dia(s) sem movimento. Por favor verifique.",
                        "data" => date("Y-m-d H:i:s"),
                        "tipo_postagem" => "Aviso de prospecção sem movimento"
                );	
                funcao_prospeccao_update($id, $dados_prospeccao, $dados_prospeccao_descricao);
                // fim - atualiza prospecção
                
                // função que envia e-mail
                email_prospeccao_sem_movimento($id, $email_para, $dados_prospeccao_descricao['tipo_postagem'], $dados_prospeccao_descricao['descricao']);
        ...
        <? mysql_free_result($prospeccao_descricoes); ?>
