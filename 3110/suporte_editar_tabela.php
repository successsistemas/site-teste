		...
		$(document).ready(function() {
			...
			$("input[name='confirmacao_recebimento']").change(function() { // ao mudar o valor do select
				...
				$("input[name='confirmacao_recebimento']:checked").each(function() {
					...
					// sim
					if (confirmacao_recebimento_atual == "sim") {
						$('#caixa_comprovante_pagamento').show();
					}
					// fim - sim

					// nao
					if (confirmacao_recebimento_atual == "nao") {
						$('#caixa_comprovante_pagamento').hide();
					}
					// fim - nao
			...
			$("#form").validate({
				rules: {
					...
					cobranca_recebimento_justificativa: {
						required: {
							depends: function() {
								return $('input[name=confirmacao_recebimento]:checked').val() == 'nao';
							}
						}
					},
					...
					confirmacao_recebimento: {
						...
						validar_comprovante_pagamento: {
							depends: function() {
								return $('input[name=confirmacao_recebimento]:checked').val() == 'sim';
							}
						}
					},
	...
	<div class="div_solicitacao_linhas2">
		<table cellspacing="0" cellpadding="0">
			<tr>
				...
				FROM usuarios 
				WHERE status = 1 and $query_suporte_responsavel 
						...
						<? if (
							...
							<div style="padding-bottom: 10px;">
								...
								if ($tipo_suporte_inloco == "cs" or $tipo_suporte_inloco == "cn") {
									$query_usuario_envolvido = sprintf(
										"
														 SELECT IdUsuario, nome, praca FROM usuarios WHERE status = 1 and praca = %s and IdUsuario != $usuario_atual ORDER BY praca, nome ASC",
										GetSQLValueString($row_suporte['praca'], "text")
									);
								}
								if ($tipo_suporte_inloco == "p") {
									$query_usuario_envolvido = "SELECT IdUsuario, nome, praca FROM usuarios WHERE status = 1 and IdUsuario != $usuario_atual ORDER BY praca, nome ASC";
								}
						...
						<? if (
							...
							<div style="padding-bottom: 10px;">
								...
								<fieldset id="confirmacao_recebimento" style="border: 1px solid #CCCCCC; padding: 5px;">
									<input name="confirmacao_recebimento" id="confirmacao_recebimento_sim" type="radio" value="sim" />
									<label for="confirmacao_recebimento_sim">sim</label>
									<input name="confirmacao_recebimento" id="confirmacao_recebimento_nao" type="radio" value="nao" />
