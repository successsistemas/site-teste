...
Vendas: <? echo $totalRows_venda_agenda_treinamento; ?>
</div>
...
<div style="border: 1px solid #CCC; margin: 5px; padding: 5px;">
	...
	<!-- dados -->
    <?
    $previsao_geral = $row_venda_agenda_treinamento['venda_agenda_treinamento_data'];
    ...
    Número da venda (treinamento): <strong><?php echo $id = $row_venda_agenda_treinamento['id']; ?></strong>
	<br>
    Praça: <strong><?php echo $row_venda_agenda_treinamento['praca']; ?></strong>
    <br>
    Data da venda (treinamento): <? echo date('d-m-Y  H:i', strtotime($row_venda_agenda_treinamento['data_venda'])); ?>
    <br>
    Situação: <strong><? echo $row_venda_agenda_treinamento['situacao']; ?></strong>    
    <br>
    Número do Agendamento: <strong><?php echo $id_agenda = $row_venda_agenda_treinamento['venda_agenda_treinamento_id_agenda']; ?></strong>
    <br>
    ...
    <!-- email_para -->
	<?
    $email_para = $row_venda_agenda_treinamento['usuario_responsavel'];
    ?> 
    <!-- fim - email_para -->
    ...
    <? if ($previsao_geral_passados_dias >= $prazo_venda_auto_email_dias){ ?>
        ...
        <!-- atualiza venda (treinamento)/envia e-mail -->
        <div style="color: #09F;">
        Envia um e-mail para: <? echo $email_para; ?>
        <br>
        Esta venda (treinamento) completou mais de 24 horas de agendamento em aberto atrasado. Por favor verifique.
        </div>
        <?
        // atualiza venda (treinamento)
        $dados_venda = array(
                "situacao" => $row_venda_agenda_treinamento['situacao']
        );	
        $dados_venda_descricao = array(
                "id_venda" => $id,
                "id_usuario_responsavel" => "",
                "descricao" => "Esta venda (treinamento) completou mais de 24 horas de agendamento em aberto atrasado. Por favor verifique.",
                "data" => date("Y-m-d H:i:s"),
                "tipo_postagem" => "Aviso de agendamento de venda (treinamento) atrasado"
        );	
        funcao_venda_update($id, $dados_venda, $dados_venda_descricao);
        // fim - atualiza venda (treinamento)
        
        // função que envia e-mail
        email_venda_agenda_treinamento_atraso($id, $id_agenda, $email_para, $dados_venda_descricao['tipo_postagem'], $dados_venda_descricao['descricao']);
        // fim - função que envia e-mail
        ?>
        <!-- fim - atualiza venda (treinamento)/envia e-mail -->
