...
SELECT 
    usuarios.praca, geral_tipo_praca_executor.praca, usuarios.IdUsuario, geral_tipo_praca_executor.IdExecutor
FROM 
    usuarios 
INNER JOIN 
    geral_tipo_praca_executor ON usuarios.praca = geral_tipo_praca_executor.praca 
WHERE 
    usuarios.IdUsuario = ".$row_usuario['IdUsuario']."
");
...
FROM 
    da01 
INNER JOIN 
    da37 ON da01.codigo1 = da37.cliente17
WHERE 
    $where_empresa and 
...
// se existe filtro de area
if( (isset($_GET["area"])) && ($_GET['area'] !="") ) {
	$colname_suporte_area = $_GET['area'];
	$where .= " and usuarios.area = '".$colname_suporte_area."' ";
	$where_campos[] = "area";
} 
// fim - se existe filtro de area
...
// se existe filtro de atendimento
$contador_atendimento = 0;
$contador_atendimento_atual = 0;
if( (isset($_GET["atendimento"])) && ($_GET['atendimento'] !="") ) {

		// contar quantidade de situacões atual
		foreach($_GET["atendimento"] as $atendimento){
				$contador_atendimento = $contador_atendimento + 1;
		}
		// fim - contar quantidade de situacões atual

		$query_atendimento=" and ( ";
		foreach($_GET["atendimento"] as $atendimento){
				$contador_atendimento_atual = $contador_atendimento_atual + 1; // verifica o contador atual
				$contador_total = $contador_atendimento - $contador_atendimento_atual; // calcula diferença de situações total - situação atual
				if($contador_total<>0){$or=" or ";}else{$or="";} // se não é a última, então insere OR
				$query_atendimento .= sprintf(" atendimento = '$atendimento' $or");

		}
		$where .= sprintf($query_atendimento)." ) ";
		$where_campos[] = "atendimento";		
}
// fim - se existe filtro de atendimento
...
SELECT 
    suporte.*, 
    suporte.data_inicio AS data_inicio2, 
...
FROM 
    suporte 
LEFT JOIN 
    usuarios ON usuarios.IdUsuario = suporte.id_usuario_responsavel 
WHERE 
    $where 
ORDER BY 
    case 
...
$(document).ready(function() {
	...
	$('#checkall_atendimento').click(function () {
		$(this).parents('fieldset:eq(0)').find(':checkbox').attr('checked', this.checked);
	});
    ...
    <div class="div_filtros">
        ...
        <tr>
            ...
            <td style="text-align:left" width="160px">
                ...
                <select name="area">
                <option value=""<?php if (!(strcmp("", isset($_GET['area'])))) {echo "selected=\"selected\"";} ?>>...</option>
                <option value="a"<?php if ( (isset($_GET['area'])) and (!(strcmp("a", $_GET['area']))) ) {echo "selected=\"selected\"";} ?>>Administrativo</option>
                <option value="o"<?php if ( (isset($_GET['area'])) and (!(strcmp("o", $_GET['area']))) ) {echo "selected=\"selected\"";} ?>>Operacional</option>
                </select>
            </td>
    ...
    <div class="div_filtros2">
        <table cellspacing="0" cellpadding="0" width="100%">
            <tr>
                <td style="text-align:left" valign="top">
                <fieldset>
                    <span class="label_solicitacao">Atendimento:</span>
                        
                    <? foreach($atendimento_array AS $atendimento_array_key => $atendimento_value){ ?>

                        <input  name="atendimento[]" type="checkbox" class="checkbox" value="<? echo $atendimento_array_key; ?>"
                            <?
                            // verificar se foi selecionada
                            if(isset($_GET['atendimento'])){
                                foreach($_GET["atendimento"] as $atendimento){
                                    if($atendimento == $atendimento_array_key){
                                        echo "checked=\"checked\"";
                                    }
                                }
                            }
                            // verificar se foi selecionada
                            ?>
                        /><? echo $atendimento_value['titulo']; ?>

                    <? } ?>

                    <input type="checkbox" class="checkbox" id="checkall_atendimento"  name="checkall_atendimento" />Marcar todos
...

{
    ...
    ?>",
    data_inicio:"<?php echo $row_suporte['data_inicio']; ?>",
    ...
    ?><? echo "</div>"; ?>",
    atendimento:"<span title='<? echo @$atendimento_array[$row_suporte['atendimento']]['titulo']; ?>' class='<? echo $cor_css; ?>'><?php echo $row_suporte['atendimento']; ?></span>",
...
jQuery('#suportes').jqGrid({
	data:dados,
	datatype: 'local',
	colNames:['Núm','In-loco','Título','Para','Cliente','Responsável','Envolvido','Início','Alterado a','Agendamento','Status','Atend.','Prior.','Situação',''],
	colModel :[ 
        ...
        {name:'data_inicio', index:'data_inicio', width:55, formatter:'date', formatter:'date', formatoptions: { srcformat: "ISO8601Long", newformat: "d-m-Y H:i" }, align:'center' },
		{name:'status', index:'status', width:40, align:'center', title:false},
        {name:'atendimento', index:'atendimento', width:40, align:'center', title:false},
...
<div id="gerar_relatorio" style="display: none;">
    ...
    <div class="div_solicitacao_linhas4">
        ...
        <input value="data_inicio2" type="checkbox" name="relatorio_campos[]" />
        Data de agendamento
        </td>
        ...
        <input value="atendimento" type="checkbox" name="relatorio_campos[]" />
        Atendimento
        <br>
